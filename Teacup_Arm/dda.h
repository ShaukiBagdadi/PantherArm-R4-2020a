...
enum axis_e { X = 0, Y, Z, U, E, AXIS_COUNT };
...
typedef struct {
  ...
  uint8_t debounce_count_x, debounce_count_y, debounce_count_z, debounce_count_u;
} MOVE_STATE;
...
typedef struct {
	...
	union {
      ...
      // uint for distance/speed calculations. --Traumflug 2014-07-04
			...
			uint8_t						u_direction   :1; ///< direction flag for U axis
