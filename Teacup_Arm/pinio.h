...
#if defined U_STEP_PIN && defined U_DIR_PIN
...
#endif
#else
#define	_u_step(x)					do { } while (0)
#define	u_step()						do { } while (0)
#define	u_direction(x)			do { } while (0)
#endif
#ifdef	U_MIN_PIN
#ifndef U_INVERT_MIN
...
#define unstep() 							do { _x_step(0); _y_step(0); _z_step(0); _u_step(0); _e_step(0); } while (0)
...
#ifdef	U_ENABLE_PIN
#ifdef	U_INVERT_ENABLE
#define	u_enable()				do { WRITE(U_ENABLE_PIN, 0); } while (0)
#define	u_disable()				do { WRITE(U_ENABLE_PIN, 1); } while (0)
#else
#define	u_enable()				do { WRITE(U_ENABLE_PIN, 1); } while (0)
#define	u_disable()				do { WRITE(U_ENABLE_PIN, 0); } while (0)
#endif
#else
#define	u_enable()					do { } while (0)
#define	u_disable()					do { } while (0)
#endif
...
inline void endstops_on(void) {
    ...
    #endif
    #ifdef U_MIN_PIN
      PULLDOWN_ON(U_MIN_PIN);
    #endif
    #ifdef U_MAX_PIN
      PULLDOWN_ON(U_MAX_PIN);
    #endif
#endif
}
