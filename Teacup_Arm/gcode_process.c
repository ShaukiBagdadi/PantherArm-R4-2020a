...
void process_gcode_command() {
	...
	#endif
  #ifdef	U_MIN
    if (next_target.target.axis[U] < (int32_t)(U_MIN * 1000.))
      next_target.target.axis[U] = (int32_t)(U_MIN * 1000.);
  #endif
  #ifdef	U_MAX
    if (next_target.target.axis[U] > (int32_t)(U_MAX * 1000.))
      next_target.target.axis[U] = (int32_t)(U_MAX * 1000.);
  #endif
	...
	if (next_target.seen_G) {
      ...
      case 112:
        ...
        break;
			...
			case 114:
				...
				sersendf_P(PSTR("X:%lq,Y:%lq,Z:%lq,U:%lq,E:%lq,F:%lu\n"),
                        current_position.axis[X], current_position.axis[Y],
                        current_position.axis[Z], current_position.axis[U],
                        current_position.axis[E],
				                current_position.F);

        if (DEBUG_POSITION && (debug_flags & DEBUG_POSITION)) {
          sersendf_P(PSTR("Endpoint: X:%ld,Y:%ld,Z:%ld,U:%ld,E:%ld,F:%lu,c:%lu}\n"),
                     movebuffer[mb_tail].endpoint.axis[X],
                     movebuffer[mb_tail].endpoint.axis[Y],
                     movebuffer[mb_tail].endpoint.axis[Z],
                     movebuffer[mb_tail].endpoint.axis[U],
          ...
          print_queue();
			...
			case 115:
				...
				sersendf_P(PSTR("FIRMWARE_NAME:Teacup FIRMWARE_URL:http://github.com/traumflug/Teacup_Firmware/ PROTOCOL_VERSION:1.0 MACHINE_TYPE:RoboArm EXTRUDER_COUNT:%d TEMP_SENSOR_COUNT:%d HEATER_COUNT:%d\n"), 1, NUM_TEMP_SENSORS, NUM_HEATERS);
				break;
      ...
      case 119:
        ...
        delay_ms(10); // allow the signal to stabilize
        {
                 ...
                 defined(U_MIN_PIN) || defined(U_MAX_PIN))
          ...
          #endif
          #if defined(U_MIN_PIN)
            serial_writestr_P(PSTR("u_min:"));
            u_min() ? serial_writestr_P(triggered) : serial_writestr_P(open);
          #endif
          #if defined(U_MAX_PIN)
            serial_writestr_P(PSTR("u_max:"));
            u_max() ? serial_writestr_P(triggered) : serial_writestr_P(open);
          #endif
        }
